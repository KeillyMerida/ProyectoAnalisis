            ...
            Using conn As New OracleConnection(My.Settings.Conexion)
                Try
                    ...
                    If countUsuario > 0 Then
                        ...
                        MessageBox.Show("El usuario ya existe en otro registro. No se puede actualizar.")
                    Else
                        ' Mostrar mensaje de confirmación
                        Dim confirmacion As DialogResult = MessageBox.Show("¿Está seguro de actualizar el usuario?", "Confirmar Actualización", MessageBoxButtons.YesNo, MessageBoxIcon.Question)

                        ' Verificar si se confirma la actualización
                        If confirmacion = DialogResult.Yes Then
                            ' Actualizar los datos del usuario en la base de datos
                            Dim queryActualizar As String = $"UPDATE usuario SET Nombre = '{txtNombre.Text}', Correo = '{txtCorreo.Text}', Rol = '{txtRol.Text}', Usuario = '{txtUsuario.Text}', Contra = '{txtContra.Text}' WHERE Id_Usuario = {idUsuario}"
                            Dim commandActualizar As New OracleCommand(queryActualizar, conn)
